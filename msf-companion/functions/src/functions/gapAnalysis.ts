...
export interface GapAnalysisDeps {
  ...
  fetchExistingGaps: () => Promise<
    Array<{ id: string; clusteredQuestion: string; category: string; frequency: number }>
  >;
  clusterQuestions: (
    questions: Array<{ question: string; category: string }>,
    existingGaps: Array<{ clusteredQuestion: string }>
  ) => Promise<
    Array<{
...
export async function analyzeGaps(
  ...
  for (const cluster of clusters) {
    const existingGap = existingGaps.find(
      (g) =>
        g.clusteredQuestion.toLowerCase() ===
    ...
    if (existingGap) {
      ...
      const status =
        cluster.gapType === "coverage_gap" ? "auto_resolving" : "open";
      const autoResolveAction =
        cluster.gapType === "coverage_gap"
...
export async function clusterWithOpenAI(
  questions: Array<{ question: string; category: string }>,
  existingGaps: Array<{ clusteredQuestion: string }>
): Promise<
  Array<{
...
Return JSON: {"clusters": [...]}`,
    ...
    if (!content) return clusterHeuristic(questions);
...
app.timer("gapAnalysis", {
  ...
  handler: async (_timer: Timer, context: InvocationContext) => {
    ...
    const deps: GapAnalysisDeps = {
      fetchFailedQuestions: async (since: Date) => {
        const result = await pool.query(
          `SELECT question, category FROM "AdvisorQuestionLog"
           WHERE "confidenceScore" < 60 AND "createdAt" >= $1`,
          [since]
        );
        return result.rows;
      },
      ...
      fetchExistingGaps: async () => {
        const result = await pool.query(
          `SELECT id, "clusteredQuestion", category, frequency
           FROM "KnowledgeGap" WHERE status = 'open'`
        );
        return result.rows.map((r: { id: string; clusteredQuestion: string; category: string; frequency: number }) => ({
          id: r.id,
      ...
      upsertGap: async (gap) => {
        ...
        await pool.query(
          `INSERT INTO "KnowledgeGap" (id, "clusteredQuestion", category, "gapType", frequency, status, "autoResolveAction", "createdAt", "updatedAt")
           VALUES ($1, $2, $3, $4, $5, $6, $7, NOW(), NOW())`,
          [id, gap.clusteredQuestion, gap.category, gap.gapType, gap.frequency, gap.status, gap.autoResolveAction ?? null]
        );
      },

      incrementGapFrequency: async (gapId: string, increment: number) => {
        await pool.query(
          `UPDATE "KnowledgeGap" SET frequency = frequency + $1, "updatedAt" = NOW()
           WHERE id = $2`,
          [increment, gapId]
        );
      },
