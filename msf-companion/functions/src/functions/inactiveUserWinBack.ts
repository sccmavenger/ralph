    ...
    for (const row of res.rows) {
      try {
        ...
        if (response.ok) {
          ...
          await pool.query(
            `INSERT INTO "ChurnIntervention" (id, "commanderId", type, channel, "riskScore", "sentAt", delivered)
             VALUES (gen_random_uuid()::text, $1, 'inactive-free-winback', 'email', 0, NOW(), true)`,
            [row.id]
          );
          sent++;
        } else {
