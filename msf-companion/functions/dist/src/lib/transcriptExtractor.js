...
exports.fetchOfficialCaptions = fetchOfficialCaptions;
exports.stripSrtFormatting = stripSrtFormatting;
exports.fetchCommunityTranscript = fetchCommunityTranscript;
exports.extractTranscript = extractTranscript;
/**
 * Fetch official captions via YouTube Data API v3.
 * Returns the transcript text or null if unavailable.
 */
async function fetchOfficialCaptions(videoId, apiKey) {
    ...
    const englishTrack = listData.items.find((item) => item.snippet.language === "en");
 ...
 */
async function fetchCommunityTranscript(videoId) {
    try {
        ...
        if (!captionXml || captionXml.trim().length === 0)
            return null;
 ...
 */
async function extractTranscript(videoId, apiKey) {
