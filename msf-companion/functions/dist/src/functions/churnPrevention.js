...
exports.buildReEngageEmailHtml = buildReEngageEmailHtml;
exports.buildRetentionEmailHtml = buildRetentionEmailHtml;
exports.buildDunningEmailHtml = buildDunningEmailHtml;
exports.buildWinBackEmailHtml = buildWinBackEmailHtml;
exports.runChurnPrevention = runChurnPrevention;
const functions_1 = require("@azure/functions");
const pgClient_js_1 = require("../lib/pgClient.js");
// ─── Risk Scoring ─────────────────────────────────────────────────────────────
function calculateRiskScore(commander) {
    let score = 0;
    const now = new Date();
    ...
    if (commander.priorUsageCount > 0) {
        ...
        if (usageDropRatio >= 0.75)
            score += 20;
        else if (usageDropRatio >= 0.5)
            score += 10;
    }
    ...
    if (commander.stripeCurrentPeriodEnd) {
        ...
        if (daysUntilExpiry <= 7)
            score += 20;
        else if (daysUntilExpiry <= 13)
            score += 10;
    }
    // 5. Past payment failures (max 10 pts)
    if (commander.paymentFailures >= 2)
        score += 10;
    else if (commander.paymentFailures >= 1)
        score += 5;
    return Math.min(score, 100);
}
// ─── Cooldown Check ───────────────────────────────────────────────────────────
function isCooldownExpired(lastSentAt, cooldownDays) {
    if (!lastSentAt)
        return true;
...
function getNudgeContent(topFeature) {
...
// ─── Email Templates ──────────────────────────────────────────────────────────
function buildReEngageEmailHtml(displayName, topFeature) {
...
function buildRetentionEmailHtml(displayName) {
    return `<html><body style="font-family: -apple-system, sans-serif; background: #0f0f23; color: #e0e0e0; padding: 0; margin: 0;">
<div style="max-width: 600px; margin: 0 auto; padding: 20px;">
<div style="text-align: center; padding: 20px 0; border-bottom: 1px solid #333;">
<div style="display: inline-block; background: #dc2626; color: white; font-weight: bold; padding: 8px 12px; border-radius: 6px; font-size: 12px;">MSF</div>
...
function buildDunningEmailHtml(displayName) {
    return `<html><body style="font-family: -apple-system, sans-serif; background: #0f0f23; color: #e0e0e0; padding: 0; margin: 0;">
<div style="max-width: 600px; margin: 0 auto; padding: 20px;">
<div style="text-align: center; padding: 20px 0; border-bottom: 1px solid #333;">
<div style="display: inline-block; background: #dc2626; color: white; font-weight: bold; padding: 8px 12px; border-radius: 6px; font-size: 12px;">MSF</div>
...
function buildWinBackEmailHtml(displayName) {
    return `<html><body style="font-family: -apple-system, sans-serif; background: #0f0f23; color: #e0e0e0; padding: 0; margin: 0;">
<div style="max-width: 600px; margin: 0 auto; padding: 20px;">
<div style="text-align: center; padding: 20px 0; border-bottom: 1px solid #333;">
<div style="display: inline-block; background: #dc2626; color: white; font-weight: bold; padding: 8px 12px; border-radius: 6px; font-size: 12px;">MSF</div>
...
async function runChurnPrevention(deps, context) {
    // Check kill switch
    ...
    const result = { scanned: commanders.length, nudged: 0, reEngaged: 0, retained: 0, winBacks: 0, skipped: 0 };
    for (const commander of commanders) {
        ...
        try {
            if (riskScore >= 70) {
                ...
                if (commander.email) {
                    await deps.sendEmail(commander.email, "We've saved your progress, Commander", buildRetentionEmailHtml(commander.displayName ?? ""));
                }
                // Create notification
                await deps.createNotification(commander.id, "Your premium features are waiting", "You've been away for a while. Your progress, saved teams, and AI Advisor history are all still here.", "/dashboard");
                ...
                if (commander.email) {
                    await deps.sendEmail(commander.email, `Commander ${commander.displayName || ""}, your roster has updates waiting`.trim(), buildReEngageEmailHtml(commander.displayName ?? "", commander.topFeature));
                }
    ...
    try {
        ...
        for (const wb of winBacks) {
            try {
                if (wb.email) {
                    await deps.sendEmail(wb.email, `Commander ${wb.displayName || ""}, your intel is still here`.trim(), buildWinBackEmailHtml(wb.displayName ?? ""));
                }
...
functions_1.app.timer("churnPrevention", {
    ...
    handler: async (_timer, context) => {
        ...
        const deps = {
            isFeatureEnabled: async (key) => {
                const res = await pool.query(`SELECT enabled FROM "FeatureFlag" WHERE key = $1`, [key]);
        ...
        `, [sevenDaysAgo, fourteenDaysAgo]);
            ...
            getLastIntervention: async (commanderId, type) => {
                const res = await pool.query(`SELECT "sentAt" FROM "ChurnIntervention" WHERE "commanderId" = $1 AND type = $2 ORDER BY "sentAt" DESC LIMIT 1`, [commanderId, type]);
            ...
            createNotification: async (commanderId, title, message, linkUrl) => {
                await pool.query(`INSERT INTO "CommanderNotification" (id, "commanderId", type, title, message, "linkUrl", read, "createdAt")
           VALUES (gen_random_uuid()::text, $1, 'churn_prevention', $2, $3, $4, false, NOW())`, [commanderId, title, message, linkUrl]);
            },
            logIntervention: async (commanderId, type, channel, riskScore, scheduledAt) => {
                await pool.query(`INSERT INTO "ChurnIntervention" (id, "commanderId", type, channel, "riskScore", "sentAt", "scheduledAt", delivered)
           VALUES (gen_random_uuid()::text, $1, $2, $3, $4, NOW(), $5, $6)`, [commanderId, type, channel, riskScore, scheduledAt ?? null, !scheduledAt]);
            },
        ...
        `);
            ...
            markDelivered: async (interventionId) => {
                await pool.query(`UPDATE "ChurnIntervention" SET delivered = true, "sentAt" = NOW() WHERE id = $1`, [interventionId]);
            },
        };
        await runChurnPrevention(deps, context);
    },
});
//# sourceMappingURL=churnPrevention.js.map