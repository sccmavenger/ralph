...
async function syncISO8(deps, context) {
    ...
    for (const entry of isoEntries) {
        ...
        docs.push((0, kbGameData_js_1.generateISO8Doc)(entry.characterName, entry.isoData));
    }
...
async function fetchISO8DataFromAPI() {
    ...
    // Fetch ISO-8 data
    const response = await fetch(`${MSF_API_BASE}/game/v1/iso8Abilities`, { headers: { "x-api-key": MSF_API_KEY, "Accept": "application/json" } });
    if (!response.ok)
        return [];
...
functions_1.app.timer("kbISO8Sync", {
...
//# sourceMappingURL=kbISO8Sync.js.map