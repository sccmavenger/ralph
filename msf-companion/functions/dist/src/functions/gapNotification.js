...
exports.sendGapNotifications = sendGapNotifications;
exports.formatDiscordEmbed = formatDiscordEmbed;
exports.formatEmailHtml = formatEmailHtml;
const functions_1 = require("@azure/functions");
const pgClient_js_1 = require("../lib/pgClient.js");
async function sendGapNotifications(deps, context) {
    const report = await deps.fetchGapReport();
    const hasContent = report.high.length > 0 ||
...
function formatDiscordEmbed(report) {
    const embeds = [];
...
functions_1.app.timer("gapNotification", {
    ...
    handler: async (_timer, context) => {
        ...
        const deps = {
            fetchGapReport: async () => {
                const gapsResult = await pool.query(`SELECT "clusteredQuestion", "gapType", frequency, status, "autoResolveAction", "resolvedAt"
           FROM "KnowledgeGap"
           WHERE status IN ('open', 'auto_resolving', 'resolved')
           ORDER BY frequency DESC`);
                const report = { high: [], medium: [], autoResolved: [], featureRequests: [] };
                for (const row of gapsResult.rows) {
                    const item = {
                        question: row.clusteredQuestion,
                    ...
                    if (row.gapType === "feature_gap") {
                        ...
                        new Date(row.resolvedAt).getTime() > Date.now() - 24 * 60 * 60 * 1000) {
...
//# sourceMappingURL=gapNotification.js.map